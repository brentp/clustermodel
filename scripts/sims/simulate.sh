...
for ns in 10 20 40; do
    for rho in 0.3 0.6; do
        for w in 0 0.8; do
            ...
            gen_cmd="python -m clustercorr simulate --n-samples $ns --rho-min $rho --linkage $linkage $meth $prefix/sim"
            fit_cmd="python scripts/gen_commands.py $prefix/sim.covs.txt $prefix/sim.meth.txt $prefix/ | bash"
